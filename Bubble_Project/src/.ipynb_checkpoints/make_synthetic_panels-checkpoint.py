...
for _, m in markets.iterrows():
    ...
    for k in range(1, N_PERIODS + 1):
        ...
        max_price = max(avg_trade_price, closing_price, opening_price) + abs(rng.normal(0, 2))
        min_price = max(1, min(avg_trade_price, closing_price, opening_price) - abs(rng.normal(0, 2)))
...
market_period["lead_price"] = market_period.groupby("market_id")["closing_price"].shift(-1)
market_period["lead_delta_p"] = market_period["lead_price"] - market_period["closing_price"]
...
rep_m_mean = market_period.groupby("repetition")["normalized_mispricing"].transform("mean")
...
for _, m in markets.iterrows():
    ...
    types = rng.choice(
        ...
        size=N_TRADERS,
        p=[shares["feedback"], shares["speculator"], shares["fundamental"], shares["other"]],
    )
    ...
    for i in range(N_TRADERS):
        ...
        wealth_mean = 1200 + 120 * m["gamified"] - 40 * m["hybrid"] + 50 * (m["repetition"] == 2)
        ...
        for _, row in mp_m.iterrows():
            ...
            feedback_ref_change = mp_m.loc[mp_m["trading_day"] == td, "delta_p"].shift(1).iloc[0] if td > 2 else np.nan
            ...
            tp_h5_rows.append({
                ...
                "feedback_hit": int(pd.notna(row["delta_p"]) and np.sign(change) == np.sign(row["delta_p"]) and change != 0),
                "speculator_hit": int(pd.notna(lead_delta) and np.sign(change) == np.sign(lead_delta) and change != 0),
                "fundamental_hit": int(np.sign(change) == -np.sign(mispricing) and change != 0),
                "repetition": m["repetition"],
                "gamified": m["gamified"],
                "hybrid": m["hybrid"],
                "treatment": m["treatment"],
            ...
            trend_beta = 0.30 + 0.35 * m["gamified"] - 0.20 * m["hybrid"] - 0.10 * (m["gamified"] * m["hybrid"])
...
# market_type_shares
market_type_shares = (
    trader_types.groupby("market_id", as_index=False)
    .agg(
...
gini_df = trader_final.groupby("market_id")["final_wealth"].apply(gini).reset_index(name="wealth_gini")
...
market_summary["surges_crashes_total"] = market_summary["n_surges"] + market_summary["n_crashes"]
market_summary["repetition2"] = (market_summary["repetition"] == 2).astype(int)

# minimal placeholder files expected by pipeline
participant_map = markets[["market_id", "treatment", "market_design", "group_composition", "gamified", "hybrid", "repetition"]].copy()
participant_map["participant_code"] = np.nan
background_panel = trader_final[["participant_code", "fin_quiz_score", "overconfidence"]].drop_duplicates()
trade_panel = market_period[["market_id", "trading_day", "avg_trade_price", "closing_price", "fundamental_value", "gamified", "hybrid", "repetition", "treatment", "market_design", "group_composition"]].copy()
trader_period = trader_period_h5[["participant_code", "market_id", "trading_day", "stock_holdings", "holding_change", "repetition", "gamified", "hybrid", "treatment", "market_design", "group_composition"]].copy()
...
print("Synthetic panel files written to generated_data/")
